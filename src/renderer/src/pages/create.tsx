...
export function Create(){
  ...
  const { isPending, mutateAsync: createCostumer} = useMutation({ mutationFn: async (data: DataMutation) => {
    
    await window.api.addCostumer({
      name: data.name,
      email: data.email,
      role: data.role,
      status: true,
  ...
  async function handleAddCostumer(e: FormEvent){
    e.preventDefault();
  ...
  return (
    <div className='flex-1 flex flex-col py-12 px-10 gap-8 overflow-y-auto'>
      <section className='flex flex-1 flex-col items-center'>
        ...
        <form className='w-full max-w-96 mt-4' onSubmit={handleAddCostumer}>
          <div className='mb-2'>
